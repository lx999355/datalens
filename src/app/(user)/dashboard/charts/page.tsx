...
import { BarChart3, Plus, Eye, Download, Heart, MessageSquare } from "lucide-react"
    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <MagneticButton href="/dashboard/charts/upload" maxOffset={6}>
            <Button variant="secondary"><Icon icon={Plus} size={16} />上传图片</Button>
          </MagneticButton>
          <MagneticButton href="/dashboard/charts/generate" maxOffset={6}>
        ...
        <>
          <StaggerReveal className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.items.map((chart) => (
              <StaggerItem key={chart.id}>
                <Link href={`/charts/${chart.id}`}>
                  <GlassCard level={2} hover className="p-5">
                    {chart.fileUrl && (
                      <div className="h-32 rounded-xl bg-white/[0.03] mb-3 overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                      <Badge variant={chart.type === "generated" ? "success" : "default"}>{chart.type === "generated" ? "生成" : "图片"}</Badge>
