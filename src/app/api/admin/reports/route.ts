...
export async function GET(request: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    prisma.report.findMany({
      ...
      skip: (page - 1) * PAGE_SIZES.reports, take: PAGE_SIZES.reports,
      include: { user: { select: { username: true } } },
    }),
    prisma.report.count({ where }),
  ])

  return NextResponse.json({ data: { items, total, page, pageSize: PAGE_SIZES.reports, totalPages: Math.ceil(total / PAGE_SIZES.reports) } })
}
